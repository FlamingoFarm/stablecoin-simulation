...
def set_initial_state(initial_coll_price: float, num_owners: int, liquidation_ratio: float):
    collateral = Collateral(initial_coll_price)

    vault_coll_balance = 20 + 400 * np.random.random(num_owners)
    debt = 0.1 + 0.8 * np.random.random(num_owners) * liquidation_ratio * initial_coll_price * vault_coll_balance
    ...
    wallet_states = [Wallet(*values) for values in zip(wallet_coll_balance, wallet_stable_coin_balance)]
